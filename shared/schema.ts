...
export const insertTravelSessionSchema = createInsertSchema(
  travelSessions
).omit({
  id: true,
  createdAt: true,
...
// Base stop schema for transit stops (has both arrival AND departure)
export const stopSchema = z.object({
  location: z.string().min(1, "Location is required"),
  arrivalTime: z.string().min(1, "Arrival time is required"),
  arrivalDate: z.string().min(1, "Arrival date is required"),
  departureTime: z.string().min(1, "Departure time is required").optional(),
  departureDate: z.string().min(1, "Departure date is required").optional(),
});
// Add custom validation for transit stop in flightDetailsSchema
export const flightDetailsSchema = z
  .object({
    ...
    stops: z
      ...
      .length(
        2,
        "Exactly 2 stops are required (transit city + final destination)"
      ),
  })
  .refine(
    (data) => {
      // First stop (transit) must have departure times
      if (
        data.stops[0] &&
        (!data.stops[0].departureTime || !data.stops[0].departureDate)
      ) {
        return false;
      }
      return true;
    ...
    {
      message:
        "Transit stop must have departure time and date for the next leg",
      path: ["stops", 0],
    }
  );
